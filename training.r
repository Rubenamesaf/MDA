...
# Determinar el número óptimo con factoextra
...
cluster_summary <- aggregate(dataset[, c("Age", "Annual_Income", "Spending_Score")],
          by = list(Cluster_KMeans = dataset$Cluster_KMeans),
          FUN = mean)
print(cluster_summary)

# Guardar resultados
dataset$Cluster_KMeans <- as.factor(dataset$Cluster_KMeans)
dataset$Cluster_Hierarchical <- as.factor(dataset$Cluster_Hierarchical)
write.csv(dataset, "Segmentacion_Clientes.csv", row.names = FALSE)

cat("Archivo Segmentacion_Clientes.csv guardado con éxito.\n")
